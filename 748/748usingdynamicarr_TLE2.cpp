...
void Node::update() {
	...
	if (this->size == 2) {
		...
		noOfNodes = ((this->q1->noOfNodes == 1 && this->q2->noOfNodes == 1 &&//check number of nodes
			...
			(this->q1->pixel == this->q2->pixel &&//check color of nodes
				this->q3->pixel == this->q4->pixel &&//even if same number of nodes (== 1)
				this->q1->pixel == this->q3->pixel)) ? 1
...
int Tree::countNodes(Node* node) {
	if (node->size == 2) {
		...
		if ((node->q1->noOfNodes == 1 && node->q2->noOfNodes == 1 &&//check number of nodes
			...
			(node->q1->pixel == node->q2->pixel &&//check color of nodes
				node->q3->pixel == node->q4->pixel &&//even if same number of nodes (== 1)
				node->q1->pixel == node->q3->pixel)) {//sub squares may have different colors
